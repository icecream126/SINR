...
def main(args):
    ...
    for kappa, mu in zip(kappas, mus):
        ...
        for i in range(pdf_values.shape[0]):
            for j in range(pdf_values.shape[1]):
                pdf_values[i, j] = vmf_pdf(
                    np.array([x[i, j], y[i, j], z[i, j]]), mu, kappa
                )
    ...
    absolute_max = max(
        abs(x_min), abs(x_max), abs(y_min), abs(y_max), abs(z_min), abs(z_max)
    )
    ax.set_xlim([-absolute_max, absolute_max])
    ax.set_ylim([-absolute_max, absolute_max])
    ax.set_zlim([-absolute_max, absolute_max])
    ...
    plt.title(
        "Multiple von Mises-Fisher Distributions on Sphere with Latitude and Longitude"
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Visualize Multiple von Mises-Fisher Distributions on Sphere."
    )
    parser.add_argument(
        "--kappa",
        type=float,
        nargs="+",
        default=[40.0, 50.0, 60.0, 70.0, 80.0],
        help="List of concentration parameters for the von Mises-Fisher distributions.",
    )
    parser.add_argument(
        "--num_points",
        type=int,
        default=5,
        help="Number of random mu points to generate.",
    )
    parser.add_argument(
        "--res", type=int, default=1, help="Resolution of latitude and longitude."
    )
    parser.add_argument(
        "--custom_fn",
        type=str,
        default="data.npz",
        help="Filename to save the data and image.",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="dataset/spatial/von_mises",
        help="Directory to save the data and image.",
    )

    args = parser.parse_args()
    args.output_dir = (
        args.output_dir + "_" + str(args.kappa) + "_" + str(args.num_points)
    )
    os.makedirs(args.output_dir, exist_ok=True)
