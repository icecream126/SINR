...
def main(l, m, res, custom_fn, output_dir):
    ...
    R = sp.sph_harm(
        m, l, PHI, THETA
    ).real  # Spherical Harmonics for the meshgrid of PHI, THETA

    lat_rad, lon_rad = np.deg2rad(lat_deg), np.deg2rad(lon_deg)
    data = {"latitude": lat_rad, "longitude": lon_rad, "target": R}
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Visualize and save data for Spherical Harmonics."
    )
    parser.add_argument(
        "--l", type=int, default=3, help="Degree of the spherical harmonics."
    )
    parser.add_argument(
        "--m", type=int, default=3, help="Order of the spherical harmonics."
    )
    parser.add_argument(
        "--res",
        type=float,
        default=1,
        help="Resolution for the latitude and longitude grid in degrees.",
    )
    parser.add_argument(
        "--custom_fn",
        type=str,
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="dataset/spatial/spherical_harmonics",
