...
if __name__=='__main__':
    ...
    with wandb.init(config = vars(args)):
        ...
        with open('./sweeps/yaml/era5_swavelet.yaml') as file:
        ...
        # Data
        train_dataset = ERA5SwaveletDataset(dataset_type='train',**dict(wandb.config))
        valid_dataset = ERA5SwaveletDataset(dataset_type='valid',**dict(wandb.config))
        test_dataset = ERA5SwaveletDataset(dataset_type='test',**dict(wandb.config))
        need_time=False
        
        train_loader = DataLoader(train_dataset, batch_size=wandb.config.batch_size, shuffle=True, num_workers=wandb.config.n_workers)
        valid_loader = DataLoader(valid_dataset, batch_size=wandb.config.batch_size, shuffle=False, num_workers=wandb.config.n_workers)
        ...
        model = SwaveletINR(need_time, input_dim, output_dim, len(train_dataset), **dict(wandb.config))
        ...
        logger = WandbLogger(
            ...
            project="SINR", 
            save_dir="experiment", 
            name='swavelet/'+str(args.dataset_dir[8:])
            )
